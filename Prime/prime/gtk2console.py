...
##### THIS FILE WAS HELLA MODIFIED BY DIVMOD, INC
##### THE DATE OF MODIFICATION WAS APRIL 26 2006
##### GPL COMPLIANCE FO' SHIZZLE
...
def gtk_console(ns, quit_cb, menu=None):
    title = '>>> Prime Python Shell'
    win = gtk.Window()
    win.set_default_size(475, 300)
    win.set_title(title)
    win.connect("delete_event", quit_cb)
    cons = Console(namespace=ns)
    if menu:
        box = gtk.VBox()
        win.add(box)
        box.show()
